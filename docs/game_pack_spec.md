...
**Q: BoardView/PlayerView에 게임별 데이터를 추가하고 싶어요.**
A: 두 클래스 모두 `data: Map<String, dynamic>` 필드를 제공한다 (기본값: `{}`). 게임별 공개/비공개 데이터를 여기에 담으면 된다. 모든 값은 JSON 직렬화 가능해야 한다.
...
return BoardView(
  ...
  data: {
    'currentPhase': 'supply',
    'stockPrices': {'company1': 7},
    'stockpiles': [{'faceUpCards': [...], 'faceDownCount': 2}],
  },
);

// PlayerView: 이 플레이어의 비공개 상태만
return PlayerView(
  ...,
  data: {
    'portfolio': {'company1': 2},     // 이 플레이어의 주식 (보안!)
    'privateInfo': {'company': 'x'},  // 이 플레이어의 내부 정보만
    'pendingFees': 1000,
  },
);
```
